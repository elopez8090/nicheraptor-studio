...
export function EbookTipTapEditor({
  projectId,
  chapterId,
  initialHtml,
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class:
          "ebook-tiptap-prose min-h-[360px] max-w-none px-6 py-8 focus:outline-none sm:px-8",
      },
    },
  });
  ...
  const handleSave = useCallback(async (source: "autosave" | "manual" = "manual") => {
    ...
    if (!navigator.onLine) {
      ...
      setSavePhase("error");
      toast.warning("Offline", "Reconnect to save chapter changes.");
      return;
    }
    ...
    try {
      const result = await withRetry(
        () =>
          safeFetchJson<{ content?: string }>("/api/save-chapter", {
            method: "POST",
            ...
            fallbackError: "Could not save chapter. Please try again.",
          }),
        { retries: 1, delayMs: 700 },
      );

      if (!result.ok) {
        const message = result.error || "Could not save chapter. Please try again.";
        setSaveError(message);
        setSavePhase("error");
        toast.toast({
          title: "Could not save chapter",
          description: message,
          variant: "error",
          action: {
            label: "Retry",
  ...
  useEffect(() => {
    ...
    const chapterChanged = syncedChapterIdRef.current !== chapterId;
    syncedChapterIdRef.current = chapterId;
  ...
  return (
    <div
      ...
      data-tiptap-target="chapter-editor"
    >
