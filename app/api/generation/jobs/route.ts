...
export async function GET(request: Request) {
  ...
  return NextResponse.json({
    job: job ? toGenerationJobPublicView(job) : null,
    activeJob: activeJob ? toGenerationJobPublicView(activeJob) : null,
...
export async function POST(request: Request) {
  try {
    ...
    console.error("POST /api/generation/jobs:", error);
...
export async function DELETE(request: Request) {
  ...
  return NextResponse.json({ ok: true, cancelled: true, jobId: active.id });
}
